...
{
    return fabs(x-t.x) < std::numeric_limits<double>::epsilon() * 100
		&& fabs(y-t.y) < std::numeric_limits<double>::epsilon() * 100
		&& fabs(z-t.z) < std::numeric_limits<double>::epsilon() * 100;
}
